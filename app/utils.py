...
def initDevDB():
    ...
    dev_user_id = crud.createPartner(db, partner=schemas.PartnerCreate(name="DEV_PARTNER"))
...
def verifyUserAuth(db: Session, user_id: int, session_id: str)->HTTPException | None:
    try:
        db.query(models.UserSession).where(models.UserSession.user_id == user_id, models.UserSession.id == session_id).one()
    except :
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="bad session id or partner id")